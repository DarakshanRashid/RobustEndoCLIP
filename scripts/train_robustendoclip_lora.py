#!/usr/bin/env python3
"""Single-dataset SurgVLP fine-tuning with LoRA on FC projection layer only.

Workflow:
1) Load SurgVLP base weights (SurgVLP.pth).
2) Freeze everything.
3) Inject LoRA on image FC projection layer (`backbone_img.global_embedder`).
4) Train only LoRA parameters on one dataset split.
5) Merge LoRA delta into FC weights and save a full model checkpoint (e.g., SurgVLP_d1.pth).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Single-dataset SurgVLP training with LoRA FC-only")
    ...
    parser.add_argument("--output-dir", type=str, default="/DATA3/SurgVLP-main/outputs/single_dataset_lora")
    parser.add_argument(
        "--save-model",
        type=str,
        default="",
...
def inject_lora_on_module(
    ...
    wrapper = LoRALinear(target, rank=rank, alpha=alpha, dropout=dropout)
...
def assert_only_lora_trainable(model: nn.Module) -> None:
    ...
    if bad:
        ...
        raise RuntimeError(
            "Non-LoRA parameters are trainable, which violates frozen-backbone training: "
            f"{preview}{tail}"
        )
...
def merge_lora_linear(wrapper: LoRALinear) -> nn.Linear:
    ...
    with torch.no_grad():
        # For y = xW^T + (xA^T)B^T * scaling, merged linear uses W' = W + (B@A)*scaling.
        delta_w = (wrapper.lora_B.weight @ wrapper.lora_A.weight) * wrapper.scaling
...
def load_lora_state(model: nn.Module, lora_state: Dict[str, torch.Tensor]) -> None:
    if not lora_state:
        raise RuntimeError("No LoRA parameters available to load.")
...
def main() -> None:
    ...
    with open(log_csv_path, "w", newline="", encoding="utf-8") as log_file:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if improved:
                ...
                best_epoch = epoch
                best_lora_state = extract_lora_state(model)
                no_improve_epochs = 0
    ...
    if best_lora_state:
        load_lora_state(model, best_lora_state)
        print(f"[Info] loaded best LoRA state from epoch {best_epoch} ({best_metric_name}={best_metric_value:.4f})")
    else:
        print("[Warn] Best LoRA state was not tracked; saving final epoch state.")

    # Merge LoRA modules into base FC module(s) and save a full plain state_dict.
    model.eval()
    merge_lora_modules_inplace(model, target_modules)
